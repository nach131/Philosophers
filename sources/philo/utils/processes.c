...
/*   Updated: 2023/09/18 16:41:14 by nmota-bu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void print_does(t_philo *philo, int type)
{
	...
	printf("%04llums ", time_elapsed());
	printf("#%02d ", philo->num);
	printf("%s\n", mss);
...
static void take_spoon(t_philo *philo)
{
	...
	// print_does(philo, TAKE);
	pthread_mutex_lock(&philo->data->m_print);
	printf("Philo num: %d\tLeft: %d\tRight: %d\n", philo->num, spoon_l, spoon_r);
	// printf(CYAN "%04llums " RESET, time_elapsed());
	// printf(MAGENTA "#%02d " RESET, philo->num);
...
void *processes(void *arg)
{
	...
	while (1)
	{
		if (philo->data->is_dead || philo->eats == philo->data->num_meals || philo->data->num_philos == 1)
			break;
		take_spoon(philo);
